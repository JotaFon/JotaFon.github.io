  ...
  useEffect(() => {
    ...
    media.add({ motion: '(prefers-reduced-motion: no-preference)', desktop: '(min-width: 761px)', pointer: '(hover: hover) and (pointer: fine)' }, ({ conditions }) => {
      ...
      const context = gsap.context(() => {
        ...
        if (desktop) {
          gsap.to('.hero-copy', { y: -65, ease: 'none', scrollTrigger: { trigger: '.hero', start: 'top top', end: 'bottom top', scrub: .7 } });
        }
        ...
        const experienceList = root.querySelector('.experience-list');
        ...
        gsap.fromTo('.reading-word', { color: '#747474' }, { color: '#202020', stagger: .08, ease: 'none', scrollTrigger: { trigger: '.reading-statement', start: 'top 83%', end: 'bottom 48%', scrub: .45 } });
        ...
        gsap.from('.contact-card', { y: 40, rotation: -3, scaleX: 1.04, scaleY: .94, opacity: 0, stagger: .08, duration: .85, ease: 'elastic.out(1, .7)', scrollTrigger: { trigger: '.contact-cards', start: 'top 96%', once: true }, clearProps: 'all' });
        ...
        if (pointer) {
          root.querySelectorAll<HTMLElement>('.contact-card').forEach((element) => {
            ...
            const settle = gsap.to(icon, { paused: true, scaleX: 1, scaleY: 1, y: 0, rotation: 0, duration: .45, ease: 'back.out(1.4)' });
            const onEnter = () => { settle.pause(); bounce.invalidate().restart(); };
            const onLeave = () => { bounce.pause(); settle.invalidate().restart(); };

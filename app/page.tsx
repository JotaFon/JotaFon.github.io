  ...
  useEffect(() => {
    ...
    media.add('(prefers-reduced-motion: no-preference)', () => {
      const context = gsap.context(() => {
        gsap.from('.hero h1 span', { y: 70, opacity: 0, duration: 1.15, stagger: .12, ease: 'power3.out', clearProps: 'all' });
        gsap.from('.hero-summary, .hero-top', { opacity: 0, y: 15, delay: .45, duration: .9, clearProps: 'all' });
        gsap.utils.toArray<HTMLElement>('[data-reveal]').forEach((element) => {
          gsap.from(element, { y: 35, opacity: 0, duration: .8, ease: 'power2.out', scrollTrigger: { trigger: element, start: 'top 92%', once: true }, clearProps: 'all' });
        });
      }, rootRef);
    ...
    <div ref={rootRef}>
      ...
      <main id="main">
        <section className="hero" id="home">
          ...
          <div className="hero-main"><Sculpture/><div className="hero-copy"><p className="eyebrow">{t('hero.eyebrow')}</p><h1><span>{t('hero.line1')}</span><span>{t('hero.line2')}</span><span className="accent">{t('hero.line3')}</span></h1></div><div className="hero-summary"><p>{t('hero.intro')}</p><a className="round-link" href="#work">{t('hero.cta')}<span><ArrowDown size={20} /></span></a></div></div>
          <div className="hero-bottom mono"><span>{t('hero.edition')}</span><span>{t('hero.scroll')}<ArrowDown size={14}/></span></div>
        </section>
        <section id="work" className="section work">
          <div className="section-heading" data-reveal><p className="eyebrow">{t('work.label')}</p><div><h2>{t('work.title')}</h2><p className="section-intro">{t('work.intro')}</p></div><ArrowDown className="heading-arrow" size={42} strokeWidth={1}/></div>
          <Accordion type="single" collapsible className="experience-list">
            {items.map(({ company, type, role, date, title, description, details, stack }, index) => (
              <AccordionItem key={`${company}-${index}`} value={`work-${index}`} className="experience" data-reveal>
        ...
        <section id="about" className="section about">
          <div className="about-heading" data-reveal><p className="eyebrow">{t('about.label')}</p><h2>{t('about.title1')}<br /><span className="accent">{t('about.title2')}</span></h2></div>
        ...
        <section id="contact" className="section contact">
          ...
          <div data-reveal><h2>{t('contact.title1')}<br /><span>{t('contact.title2')}</span></h2><p className="contact-intro">{t('contact.text')}</p><div className="email-row"><a href="mailto:jotaafon@gmail.com">{t('contact.email')}<ArrowUpRight strokeWidth={1}/></a><button onClick={copyEmail} aria-label={t('contact.copy')}>{copyStatus === 'copied' ? <Check size={20}/> : <Copy size={20}/>}</button></div><p className="copy-status mono" role="status">{copyStatus ? t(`contact.${copyStatus}`) : '\u00a0'}</p></div>
